...
class TrainingRequestForm(forms.ModelForm):
  ...
  course = forms.ModelChoiceField(empty_label='---------', queryset=CourseMap.objects.filter(category=0).select_related('course', 'foss'))
  batch = forms.ModelChoiceField(empty_label='---------', queryset=StudentBatch.objects.none())
  fossmdlmap = forms.ModelChoiceField( label="Test Type", queryset=FossMdlCourses.objects.all().select_related('language', 'level'), required=False, empty_label='---------',)
