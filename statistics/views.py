...
from events.filters import TrainingRequestFilter, \
  TestFilter, AcademicCenterFilter
...
def training(request):
  ...
  #
  chart_query_set = collection.qs.extra(select={'year': "EXTRACT(year FROM sem_start_date)"}).values('year').order_by('-year').annotate(total_training=Count('sem_start_date'), total_participant=Sum('participants'))
  chart_data = ''
...
def test_participant(request, rid):
  ...
  try:
    ...
    test_mdlusers = TestAttendance.objects.using('default').filter(test_id=rid, status__gte=2).values_list('mdluser_id')
    ids = []
