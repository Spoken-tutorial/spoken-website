...
urlpatterns = [
  ...
  url(r'api/tutorial/(?P<tid>[0-9]+)/language/(?P<lid>[0-9]+)/scripts/(?P<script_detail_id>[0-9]+)/$', views.ScriptCreateAPIView.as_view()),
  ...
  url(r'api/scripts/(?P<script_detail_id>[0-9]+)/reversions/(?P<reversion_id>[0-9]+)/$', views.ReversionListView.as_view()),
