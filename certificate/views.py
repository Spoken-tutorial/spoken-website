...
def koha_12octrc_certificate_download(request):
    context = {}
    err = ""
    ci = RequestContext(request)
    cur_path = os.path.dirname(os.path.realpath(__file__))
    ...
    if request.method == 'POST':
        ...
        if  rccemail:
            email = rccemail
            user = Koha_RC_12oct2018.objects.filter(email=email, usertype='rcc')
            workshop = 'rcc'
        elif ccemail:
            email = ccemail
            user = Koha_RC_12oct2018.objects.filter(email=email, usertype='cc')
            workshop = 'cc'
        elif saemail:
            email = saemail
            user = Koha_RC_12oct2018.objects.filter(email=email, usertype='sa')
            workshop = 'sa'
...
def create_koha_12oct_rc_certificate(certificate_path, name, qrcode, type, paper, workshop, file_name):
    ...
    try:
        ...
        if workshop == 'cc':
            usertype = 'Course Coordinator'
            download_file_name = 'KMW12102018CCcertificate.pdf'

        if workshop == 'sa':
            usertype = 'System Administrator'
            download_file_name = 'KMW12102018SAcertificate.pdf'
