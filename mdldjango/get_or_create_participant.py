...
def get_or_create_participant(w, firstname, lastname, gender, email, category):
    print firstname
    mdluser = None
    # No email for school students
    if w and w.organiser.academic.institution_type.name == 'School':
        sta = SchoolTrainingAttendance()
        sta.status = 1
        sta.training_id = w.id
        sta.firstname = firstname
        sta.lastname = lastname
        sta.gender = gender
        sta.email = email
        sta.save()
        return True
