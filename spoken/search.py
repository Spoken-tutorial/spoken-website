...
def search_for_results(userquery, corrected_flag=True):
	try:
		if os.path.exists(settings.SEARCH_INDEX_DIR):
			...
			#OrGroup.factory - which is useful better for giving relavance rather than naive term frequency of the words in the query
			og = qparser.OrGroup.factory(0.9)

			#initializing Multifield Parser for searching in the multiple fields
			queryparser = qparser.MultifieldParser(["tags","foss","title","outline"], schema=index_reference.schema, group=og)
