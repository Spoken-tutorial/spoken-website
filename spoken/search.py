...
# Function used to search the query in the index table and display search results
# and related searches for the user
...
def search_for_results(userquery, corrected_flag=True):
	try:
		if os.path.exists(settings.SEARCH_INDEX_DIR):
			...
			#OrGroup.factory - which is useful better for giving relavance
			# rather than naive term frequency of the words in the query
			...
			queryparser = qparser.MultifieldParser(
				...
				schema=index_reference.schema, group=og)
			...
			# can specify a fuzzy term by adding a ~ followed
			# by an optional maximum edit distance (Ex : jav~1)
			...
			# For Correcting Spelling with maximum edit distance 3.
			# More than 3 It may affect the performance.
			...
			if (results.has_matched_terms() and results.scored_length() > 0):
				collection = []
				for hit in results:
					row = TutorialResource.objects.filter(tutorial_detail_id=hit['VideoId'], language__name = 'English').first()
					collection.append(row)
				return collection, corrected_string
			else:
				return None, corrected_string
				"""print("Similar Searches:")
				for subhit in results[0].more_like_this("outline", top=5):
					print(subhit["title"])"""
