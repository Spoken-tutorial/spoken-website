...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        domain_roles = \
            DomainReviewerRole.objects.filter(status=1).values('user_id', 'user__username', 'language_id', 'language__name').distinct()
        quality_roles = \
            QualityReviewerRole.objects.filter(status=1).values('user_id', 'user__username', 'language_id', 'language__name').distinct()
