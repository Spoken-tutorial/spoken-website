...
def create_thumbnail(row, attach_str, thumb_time, thumb_size):
    filepath = settings.MEDIA_ROOT + 'videos/' + \
        str(row.tutorial_detail.foss_id) + '/' + str(row.tutorial_detail_id) + '/'
    filename = row.tutorial_detail.tutorial.replace(' ', '-') + '-' + attach_str + '.png'
    try:
        process = subprocess.Popen(['/usr/bin/ffmpeg', '-i', filepath + row.video, '-r', str(30), '-ss',
                                    str(thumb_time), '-s', thumb_size, '-vframes', str(1), '-f', 'image2',
                                    filepath + filename],
...
def add_qualityreviewer_notification(tr_rec, comp_title, message):
    dr_roles = QualityReviewerRole.objects.filter(
        foss_category=tr_rec.tutorial_detail.foss, language=tr_rec.language, status=1)
    for dr_role in dr_roles:
        QualityReviewerNotification.objects.create(
            user=dr_role.user, title=comp_title, message=message, tutorial_resource=tr_rec)


def add_domainreviewer_notification(tr_rec, comp_title, message):
    dr_roles = DomainReviewerRole.objects.filter(
        foss_category=tr_rec.tutorial_detail.foss, language=tr_rec.language, status=1)
    for dr_role in dr_roles:
        DomainReviewerNotification.objects.create(
            user=dr_role.user, title=comp_title, message=message, tutorial_resource=tr_rec)
...
def add_contributor_notification(tr_rec, comp_title, message):
    con_roles = ContributorRole.objects.filter(
        foss_category=tr_rec.tutorial_detail.foss, language=tr_rec.language, status=1)

    for con in con_roles:
        ContributorNotification.objects.create(user=con.user, title=comp_title,
                                               message=message, tutorial_resource=tr_rec)
    ...
    if role_type in roles:
        try:
            ...
            try:
                ...
                flag = 0
                messages.warning(request, 'Request to the ' + role_type.title() +
                                 ' role is already waiting for admin approval!')
    else:
        flag = 0
        ...
        try:
            ...
            if role_rec.role_type in roles:
                try:
                    ...
                    messages.error(request, role_rec.user.username + ' is already having ' +
                                   roles[role_rec.role_type] + ' role.')
            else:
                messages.error(request, 'Invalid role argument!')
        except:
...
def creationhome(request):
    if is_contributor(request.user) or is_domainreviewer(request.user) or \
            is_videoreviewer(request.user) or is_qualityreviewer(request.user):
        contrib_notifs = []
        admin_notifs = []
        domain_notifs = []
        quality_notifs = []
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if TutorialCommonContent.objects.filter(tutorial_detail_id=request.POST['tutorial_name']).count():
                ...
                common_content.save()
            if TutorialResource.objects.filter(tutorial_detail_id=request.POST['tutorial_name'],
                                               common_content_id=common_content.id,
                                               language_id=request.POST['language']).count():
                tutorial_resource = TutorialResource.objects.get(
                    tutorial_detail_id=request.POST['tutorial_name'],
                    common_content_id=common_content.id, language_id=request.POST['language'])
            else:
...
def upload_publish_outline(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            tutorial_resource = TutorialResource.objects.get(
                tutorial_detail_id=request.POST['tutorial_name'], language_id=request.POST['language'])
    ...
    if request.method == 'POST':
        ...
        if foss and lang and publish:
            ...
            languages = Language.objects.filter(id__in=ContributorRole.objects.filter(
                user_id=request.user.id, foss_category_id=foss).values_list('language_id'))
...
def upload_tutorial(request, trid):
    ...
    try:
        ...
        ContributorRole.objects.get(
            user_id=request.user.id, foss_category_id=tr_rec.tutorial_detail.foss_id,
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            try:
                ...
                tr_rec.save()
                ContributorLog.objects.create(status=prev_state, user=request.user,
                                              tutorial_resource=tr_rec, component='outline')
                comp_title = tr_rec.tutorial_detail.foss.foss + ': ' + tr_rec.tutorial_detail.tutorial + \
...
def upload_script(request, trid):
    ...
    response_msg = ''
    error_msg = ''
    storage_path = tr_rec.tutorial_detail.foss.foss.replace(' ', '-') + '/' + tr_rec.tutorial_detail.level.code + \
        '/' + tr_rec.tutorial_detail.tutorial.replace(' ', '-') + '/' + tr_rec.language.name
        ...
        if form.is_valid():
            try:
                ...
                if(int(code) == 200):
                    ...
                    tr_rec.save()
                    ContributorLog.objects.create(status=prev_state, user=request.user,
                                                  tutorial_resource=tr_rec, component='script')
                    comp_title = tr_rec.tutorial_detail.foss.foss + ': ' + tr_rec.tutorial_detail.tutorial + ' - ' + \
                        tr_rec.language.name
...
def save_timed_script(request, tdid):
    ...
    try:
        ...
        print e
        raise PermissionDenied()
    storage_path = tr_rec.tutorial_detail.foss.foss.replace(
        ' ', '-') + '/' + tr_rec.tutorial_detail.level.code + '/' + tr_rec.tutorial_detail.tutorial.replace(' ', '-') + '/' + tr_rec.language.name + '-timed'
    script_path = settings.SCRIPT_URL + storage_path
    form = UploadScriptForm(script_path)
    if request.method == 'POST':
        form = UploadScriptForm(script_path, request.POST)
        if form.is_valid():
            try:
                ...
                if(int(code) == 200):
                    ...
                    tr_rec.save()
                    srt_file_path = settings.MEDIA_ROOT + 'videos/' + str(tr_rec.tutorial_detail.foss_id) + '/' + str(
                        tr_rec.tutorial_detail_id) + '/' + tr_rec.tutorial_detail.tutorial.replace(' ', '-') + '-English.srt'
                    minified_script_url = settings.SCRIPT_URL.strip(
                        '/') + '?title=' + quote(storage_path) + '&printable=yes'
                    if generate_subtitle(minified_script_url, srt_file_path):
                        ...
                        messages.success(
                            request, 'Timed script updated successfully! But there is a in generating subtitle file.')
...
def ajax_upload_timed_script(request):
    ...
    foss = request.POST.get('foss', '')
    if foss:
        rows = TutorialDetail.objects.filter(id__in=TutorialResource.objects.filter(
            tutorial_detail__foss_id=foss, language__name='English', script_status=4).values_list('tutorial_detail_id')).order_by('order')
...
def upload_prerequisite(request, trid):
    ...
    try:
        tr_rec = TutorialResource.objects.get(pk=trid, status=0)
        ContributorRole.objects.get(
            user_id=request.user.id, foss_category_id=tr_rec.tutorial_detail.foss_id, language_id=tr_rec.language_id, status=1)
    except Exception:
        raise PermissionDenied()
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            try:
                ...
                ContributorLog.objects.create(status=prev_state, user=request.user,
                                              tutorial_resource=tr_rec, component='prerequisite')
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            try:
                ...
                tr_rec.common_content.save()
                ContributorLog.objects.create(status=prev_state, user=request.user,
                                              tutorial_resource=tr_rec, component='keyword')
                comp_title = tr_rec.tutorial_detail.foss.foss + ': ' + tr_rec.tutorial_detail.tutorial + ' - ' + tr_rec.language.name
            ...
            if form.is_valid():
                try:
                    ...
                    if component == 'video':
                        ...
                        file_name = tr_rec.tutorial_detail.tutorial.replace(
                            ' ', '-') + '-' + tr_rec.language.name + file_extension
                        file_path = settings.MEDIA_ROOT + 'videos/' + \
                            str(tr_rec.tutorial_detail.foss_id) + '/' + str(tr_rec.tutorial_detail.id) + '/'
                        ...
                        if os.path.isfile(file_path + tr_rec.video) and tr_rec.video_status > 0:
                            if 'isarchive' in request.POST and int(request.POST.get('isarchive', 0)) > 0:
                                archived_file = 'Archived-' + str(request.user.id) + \
                                    '-' + str(int(time.time())) + '-' + tr_rec.video
                                os.rename(file_path + tr_rec.video, file_path + archived_file)
                                ArchivedVideo.objects.create(
                                    tutorial_resource=tr_rec, user=request.user, version=tr_rec.version, video=archived_file, atype=tr_rec.video_status)
                        ...
                        if not tr_rec.version:
                            tr_rec.version = 1
                        tr_rec.video_thumbnail_time = '00:' + \
                            request.POST.get('thumb_mins', '00') + ':' + request.POST.get('thumb_secs', '00')
                        tr_rec.save()
                        ...
                        file_path = settings.MEDIA_ROOT + 'videos/' + \
                            ...
                            str(tr_rec.tutorial_detail.id) + '/resources/' + file_name
                        ...
                        add_domainreviewer_notification(
                            tr_rec, comp_title, component.title() + ' waiting for domain review')
...
def mark_notrequired(request, trid, tcid, component):
    ...
    try:
        ...
        if getattr(tcc, component + '_status') == 0:
            ...
            tcc.save()
            ContributorLog.objects.create(user=request.user, tutorial_resource_id=trid,
                                          component=component, status=prev_state)
    ...
    if is_contributor(request.user):
        ...
        for foss_contrib in foss_contrib_list:
            tr_recs = TutorialResource.objects.filter(
                tutorial_detail__foss_id=foss_contrib.foss_category_id, language_id=foss_contrib.language_id)
            for tr_rec in tr_recs:
                flag = 1
                if tr_rec.language.name == 'English':
                    if (tr_rec.common_content.slide_user_id != request.user.id or tr_rec.common_content.slide_status == 0) and \
                            (tr_rec.common_content.code_user_id != request.user.id or tr_rec.common_content.code_status == 0) and \
                            (tr_rec.common_content.assignment_user_id != request.user.id or tr_rec.common_content.assignment_status == 0) and \
                            (tr_rec.common_content.keyword_user_id != request.user.id or tr_rec.common_content.keyword_status == 0):
                        flag = 0
                else:
                    flag = 0
        ...
        if form.is_valid():
            ...
            if request.POST['video_status'] == '2':
                ...
                try:
                    ...
                    tr.video_status = 5
                    tr.save()
                    NeedImprovementLog.objects.create(user=request.user,
                                                      tutorial_resource=tr,
...
def admin_reviewed_video(request):
    ...
    try:
        ...
        collection = TutorialResource.objects.filter(id__in=AdminReviewLog.objects.filter(
            user=request.user).values_list('tutorial_resource_id').distinct())
        raw_get_data = request.GET.get('o', None)
        collection = get_sorted_list(request, collection, header, raw_get_data)
        ordering = get_field_index(raw_get_data)
        page = request.GET.get('page')
...
def tutorials_needimprovement(request):
    ...
    for rec in con_roles:
        tr_recs = TutorialResource.objects.filter(
            tutorial_detail__foss_id=rec.foss_category_id, language_id=rec.language_id, status=0)
        for tr_rec in tr_recs:
            flag = 1
            if tr_rec.language.name == 'English':
...
def domain_review_index(request):
    ...
    for rec in dr_roles:
        tr_recs = TutorialResource.objects.filter(tutorial_detail_id__in=TutorialDetail.objects.filter(
            foss_id=rec.foss_category_id).values_list('id'), language_id=rec.language_id, status=0).order_by('updated')
        for tr_rec in tr_recs:
            flag = 1
            if tr_rec.language.name == 'English':
            ...
            if request.POST['component_status'] == '3':
                try:
                    ...
                    if component == 'outline' or component == 'script' or component == 'video':
                        ...
                        if tr.language.name == 'English':
                            ...
                            execFlag = 1
                    if execFlag:
                        DomainReviewLog.objects.create(status=3, component=component,
                                                       user=request.user, tutorial_resource=tr)
                        add_qualityreviewer_notification(
                            tr, comp_title, component.title() + ' waiting for Quality review')
                ...
                try:
                    ...
                    if component == 'outline' or component == 'script' or component == 'video':
                        ...
                        tr.common_content.save()
                    NeedImprovementLog.objects.create(
                        user=request.user, tutorial_resource=tr, review_state=prev_state, component=component, comment=request.POST['feedback'])
                    DomainReviewLog.objects.create(status=5, component=component,
                                                   user=request.user, tutorial_resource=tr)
...
def domain_reviewed_tutorials(request):
    ...
    try:
        ...
        collection = TutorialResource.objects.filter(id__in=DomainReviewLog.objects.filter(
            user=request.user).values_list('tutorial_resource_id').distinct())
        collection = get_sorted_list(request, collection, header, raw_get_data)
        ordering = get_field_index(raw_get_data)
        page = request.GET.get('page')
...
def accept_all(request, review, trid):
    ...
    if tr.outline_status > 0 and tr.outline_status == current_status:
        ...
        if review == 'quality':
            QualityReviewLog.objects.create(
                status=status_flag[review], component='outline', user=request.user, tutorial_resource=tr)
            comp_message = 'Outline accepted by Quality reviewer'
        else:
            DomainReviewLog.objects.create(
                status=status_flag[review], component='outline', user=request.user, tutorial_resource=tr)
    ...
    if tr.script_status > 0 and tr.script_status == current_status:
        ...
        if review == 'quality':
            QualityReviewLog.objects.create(
                status=status_flag[review], component='script', user=request.user, tutorial_resource=tr)
    ...
    if tr.video_status > 0 and tr.video_status == current_status:
        ...
        if review == 'quality':
            QualityReviewLog.objects.create(
                status=status_flag[review], component='video', user=request.user, tutorial_resource=tr)
            comp_message = 'Video accepted by Quality reviewer'
        else:
            DomainReviewLog.objects.create(status=status_flag[review],
                                           component='video', user=request.user, tutorial_resource=tr)
    ...
    if tr.language.name == 'English':
        if tr.common_content.slide_status > 0 and tr.common_content.slide_status == current_status:
            ...
            if review == 'quality':
                QualityReviewLog.objects.create(
                    status=status_flag[review], component='slide', user=request.user, tutorial_resource=tr)
        ...
        if tr.common_content.code_status > 0 and tr.common_content.code_status == current_status:
            ...
            if review == 'quality':
                QualityReviewLog.objects.create(
                    status=status_flag[review], component='code', user=request.user, tutorial_resource=tr)
        ...
        if tr.common_content.assignment_status > 0 and tr.common_content.assignment_status == current_status:
            ...
            if review == 'quality':
                QualityReviewLog.objects.create(
                    status=status_flag[review], component='assignment', user=request.user, tutorial_resource=tr)
        ...
        if tr.common_content.prerequisite_status > 0 and tr.common_content.prerequisite_status == current_status:
            ...
            if review == 'quality':
                QualityReviewLog.objects.create(
                    status=status_flag[review], component='prerequisite', user=request.user, tutorial_resource=tr)
        ...
        if tr.common_content.keyword_status > 0 and tr.common_content.keyword_status == current_status:
            ...
            if review == 'quality':
                QualityReviewLog.objects.create(
                    status=status_flag[review], component='keyword', user=request.user, tutorial_resource=tr)
...
def quality_review_index(request):
    ...
    for rec in qr_roles:
        if rec.language.name == 'English':
            tr_recs = TutorialResource.objects.filter(Q(outline_status=3) | Q(script_status=3) | Q(video_status=3) | Q(common_content__slide_status=3) | Q(common_content__code_status=3) | Q(
                common_content__assignment_status=3) | Q(common_content__keyword_status=3) | Q(common_content__prerequisite_status=3), Q(tutorial_detail__foss_id=rec.foss_category_id) & Q(language_id=rec.language_id) & Q(status=0))
        else:
            tr_recs = TutorialResource.objects.filter(Q(outline_status=3) | Q(script_status=3) | Q(video_status=3), Q(
                tutorial_detail__foss_id=rec.foss_category_id) & Q(language_id=rec.language_id) & Q(status=0)).order_by('updated')
...
def publish_tutorial_index(request):
    ...
    for rec in qr_roles:
        if rec.language.name == 'English':
            tr_recs = TutorialResource.objects.filter(Q(common_content__code_status=4) | Q(common_content__code_status=6), Q(common_content__assignment_status=4) | Q(common_content__assignment_status=6), Q(common_content__prerequisite_status=4) | Q(
                common_content__prerequisite_status=6), Q(outline_status=4) & Q(script_status=4) & Q(video_status=4) & Q(common_content__slide_status=4) & Q(common_content__keyword_status=4) & Q(tutorial_detail__foss_id=rec.foss_category_id) & Q(language_id=rec.language_id) & Q(status=0))
        else:
            tr_recs = TutorialResource.objects.filter(Q(outline_status=4) & Q(script_status=4) & Q(video_status=4) & Q(
                tutorial_detail__foss_id=rec.foss_category_id) & Q(language_id=rec.language_id) & Q(status=0)).order_by('updated')
...
def public_review_tutorial_index(request):
    ...
    for rec in qr_roles:
        if rec.language.name != 'English':
            tr_recs = TutorialResource.objects.filter(Q(outline_status__gt=0) & Q(outline_status__lt=5), Q(script_status__gt=0) & Q(script_status__lt=5), Q(
                video_status__gt=0) & Q(video_status__lt=5), Q(tutorial_detail__foss_id=rec.foss_category_id) & Q(language_id=rec.language_id) & Q(status=0)).order_by('updated')
...
def public_review_list(request):
    ...
    for rec in qr_roles:
        tr_recs = TutorialResource.objects.filter(
            tutorial_detail__foss_id=rec.foss_category_id, language_id=rec.language_id, status=2).order_by('updated')
        for tr_rec in tr_recs:
            tmp_ids.append(tr_rec.id)
    collection = None
    header = ''
...
def public_review_mark_as_pending(request, trid):
    ...
    try:
        ...
        tr_rec.status = 0
        tr_rec.save()
        add_contributor_notification(
            tr_rec, comp_title, 'This tutorial moved from public review to regular review process.')
            ...
            if request.POST['component_status'] == '4':
                try:
                    ...
                    if execFlag:
                        ...
                        QualityReviewLog.objects.create(status=4, component=component,
                                                        user=request.user, tutorial_resource=tr)
                        add_contributor_notification(tr, comp_title, comp_message)
                        response_msg = 'Review status updated successfully!'
                    else:
                ...
                try:
                    ...
                    if execFlag:
                        ...
                        QualityReviewLog.objects.create(status=5, component=component,
                                                        user=request.user, tutorial_resource=tr)
                        add_contributor_notification(tr, comp_title, comp_message)
                        response_msg = 'Review status updated successfully!'
                    else:
...
def quality_reviewed_tutorials(request):
    ...
    try:
        ...
        collection = TutorialResource.objects.filter(id__in=QualityReviewLog.objects.filter(
            user=request.user).values_list('tutorial_resource_id').distinct())
        collection = get_sorted_list(request, collection, header, raw_get_data)
        ordering = get_field_index(raw_get_data)
        page = request.GET.get('page')
...
def clear_creation_notification(request, notif_type):
    try:
        if notif_type == "contributor":
            ContributorNotification.objects.filter(user=request.user).delete()
        elif notif_type == "admin":
            AdminReviewerNotification.objects.filter(user=request.user).delete()
        elif notif_type == "domain":
            DomainReviewerNotification.objects.filter(user=request.user).delete()
        elif notif_type == "quality":
            QualityReviewerNotification.objects.filter(user=request.user).delete()
    except:
...
def creation_view_tutorial(request, foss, tutorial, lang):
    ...
    try:
        ...
        tr_recs = TutorialResource.objects.filter(tutorial_detail__in=TutorialDetail.objects.filter(
            foss=tr_rec.tutorial_detail.foss).order_by('order').values_list('id'), language=tr_rec.language)
    except Exception, e:
        messages.error(request, str(e))
...
def creation_change_published_to_pending(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            try:
                row = TutorialResource.objects.get(tutorial_detail_id=request.POST.get(
                    'tutorial_name'), language_id=request.POST.get('language'))
    ...
    if request.method == 'POST':
        ...
        if foss and lang:
            ...
            tutorials = TutorialResource.objects.filter(
                tutorial_detail_id__in=td_list, language_id=lang, status=1).distinct().order_by('tutorial_detail__tutorial')
            for tutorial in tutorials:
                data += '<option value="' + str(tutorial.tutorial_detail.id) + '">' + \
                    tutorial.tutorial_detail.tutorial + '</option>'
            if data:
                data = '<option value="">Select Tutorial</option>' + data
        elif foss:
            languages = Language.objects.filter(id__in=TutorialResource.objects.filter(tutorial_detail__in=TutorialDetail.objects.filter(
                foss_id=foss).values_list('id'), status=1).values_list('language_id').distinct())
            for language in languages:
                data += '<option value="' + str(language.id) + '">' + language.name + '</option>'
            if data:
                data = '<option value="">Select Language</option>' + data
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            try:
                ...
                add_contributor_notification(row, comp_title, component.title() +
...
def ajax_change_component_status(request):
    ...
    if request.method == 'POST':
        ...
        if foss and lang and tut and comp:
            ...
            tutorials = TutorialResource.objects.filter(
                tutorial_detail_id__in=td_list, language_id=lang, status=0).distinct()
            ...
            for tutorial in tutorials:
                data[0] += '<option value="' + str(tutorial.tutorial_detail.id) + \
                    '">' + tutorial.tutorial_detail.tutorial + '</option>'
            if lang_rec.name == 'English':
            ...
            languages = Language.objects.filter(id__in=TutorialResource.objects.filter(tutorial_detail__in=TutorialDetail.objects.filter(
                foss_id=foss).values_list('id'), status=0).values_list('language_id').distinct())
            for language in languages:
                data += '<option value="' + str(language.id) + '">' + language.name + '</option>'
            if data:
                data = '<option value="">Select Language</option>' + data
        ...
        if form.is_valid():
            ...
            if report_type:
                ...
                if compStatus == 6:
                    flag = 1
                    messages.warning(
                        request, 'The contributor of this tutorial says that the selected component is not required. However if you wish to report an error, please click on "Some content is missing" radio button.')
                elif compValue:
                    if component == 1:
                        flag = 1
                        messages.warning(
                            request, 'The selected component is available. However if you wish to report an error, please click on "Some content is missing" radio button.')
                    if component <= 3:
            ...
            if inform_me and request.user.is_authenticated() is False:
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            TutorialMissingComponentReply.objects.create(
                missing_component=tmc_row, user=request.user, reply_message=request.POST.get('reply_message', ''))
...
def report_missing_component_list(request):
    ...
    if is_administrator(request.user):
        ...
        contrib_roles = list(ContributorRole.objects.filter(
            user=request.user).values_list('foss_category_id', 'language_id'))
...
def suggest_topic(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, ('Your suggestion for the Topic has been submitted to concerned person. '
...
def suggest_example(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, ('Your suggestion for the Example has been submitted to concerned person. '
                                       'We\'ll contact you soon.'))
    else:
        form = SuggestExampleForm()
    context = {
...
def collaborate(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, ('Form has been submitted to concerned person. Thank you for showing interest '
                                       'on collaborate with us. We\'ll contact you soon.'))
    else:
        form = CollaborateForm()
    context = {
        'form': form
        ...
        if form.is_valid():
            try:
                source_tutorial = TutorialDetail.objects.get(
                    pk=form.cleaned_data['source_tutorial'], foss_id=form.cleaned_data['source_foss'])
                ...
                if int(form.cleaned_data['destination_tutorial']) == 0:
                    ...
                    messages.success(request, 'Prerequisite for <b>' + source_tutorial.tutorial +
                                     '</b> updated to <b>Not Required</b>')
                else:
                    destination_tutorial = TutorialDetail.objects.get(
                        pk=form.cleaned_data['destination_tutorial'], foss_id=form.cleaned_data['destination_foss'])
                    tcc.prerequisite_id = destination_tutorial.id
                    tcc.prerequisite_status = 4
                    messages.success(request, 'Prerequisite <b>' + destination_tutorial.tutorial +
                                     '</b> updated to <b>' + source_tutorial.tutorial + '</b>.')
                tcc.save()
