...
def CommandsForOGV(filename):
     cli = [None]*6
     cli[0] = 'ffmpeg -i ' + ' ' + filename + ' -qscale 0 -an ' + 'noise.wmv'
     cli[1] = 'ffmpeg -i ' + ' ' + filename + ' -qscale 0 ' + 'nn.wav'
     ...
     cli[4] = 'ffmpeg -i nnf.wav -i noise.wmv -qscale 0 combined.wmv'
...
def CommandsForWAV(filename):
    ...
    cli[2] = 'ffmpeg -i '+filename[0:-4]+'-nonoise.wav'+' '+filename[0:-4]+'-nonoise.ogg'
